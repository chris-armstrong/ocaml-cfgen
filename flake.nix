    ...
    flake-utils.lib.eachDefaultSystem (system:
        ...
        sources = {
          ocaml = nix-filter.lib {
            ...
            include = [
              ".ocamlformat"
              "dune-project"
              (nix-filter.lib.inDirectory "definitions")
              (nix-filter.lib.inDirectory "doc")
              (nix-filter.lib.inDirectory "examples")
              (nix-filter.lib.inDirectory "generate")
              (nix-filter.lib.inDirectory "lib")
              (nix-filter.lib.inDirectory "misc")
              (nix-filter.lib.inDirectory "parse")
              (nix-filter.lib.inDirectory "test")
              (nix-filter.lib.inDirectory "write_specs")
            ];
          };
      ...
      in
      {
        ...
        packages = {
          ...
          #     $ nix run -- <args?>
          #
          default = self.packages.${system}.cfgen;

          cfgen = ocamlPackages.buildDunePackage {
            pname = "cfgen";
            version = "0.1.0";
            duneVersion = "3";
            src = sources.ocaml;
            ...
            preBuild = ''
              dune build cfgen.opam
            '';

            meta = {
              description = "AWS CloudFormation generation for OCaml";
        ...
        #
        checks = {
          # Run tests for the `cfgen` package
          cfgen =
            let
