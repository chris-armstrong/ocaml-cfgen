...
let to_safe_name str =
  ...
  | "properties" | "attributes" | "List" | "String" | "Map" ->
      str ^ "_"
  | _ -> str
...
let to_attribute_name x =
  x
  |> Containers.String.replace ~which:`All ~sub:"." ~by:"_"
  |> to_snake_case |> to_safe_name

let to_attributes_type_name resource_name =
  (resource_name |> to_type_name) ^ "_attributes"
